        ...
        <section class="break-point">
                                                ...
                                                @if ($t)
                                                    <div class="day-group">
                                                        <label class="item-check d-block day-check-block">
                                                            ...
                                                            (($disable && in_array($d->date, $backs)) || in_array($d->date,$go_back))? 'checked':''
                                                            }}>
                                                            <span></span>
            ...
            @if ($errors->has('back_date'))
                <div class="alert alert-danger text-center">
                    {{$errors->first('back_date')}}
                </div>
            @endif
        </section>
        <div class="check-all {{!$days_available? 'disabled':''}}">
            <label class="item-check d-block">
                <input type="checkbox" name="check-_all_in_date" class="check-item checkbox" {{!$days_available? 'disabled':''}}>
                <span></span>
...
@if ($route->type == 'direct' || ($route->type=='bus' && in_array(get_station_id(), [$route->from_id, $route->to_id])))
    <div class="form-group row {{$st_time_error?'has-error':''}}" id="form-group-back_trips-{{$i}}-started">
        <label for="back_trips-{{$i}}-started" class="form-control-label col-sm-4 col-md-12 col-lg-4 col-xl-3" id="label-for-back_trips{{$i}}-started">Xuất bến</label>
        <div class="input-datetime-wrapper col-sm-8 col-md-12 col-lg-8 col-xl-9">
            <input type="hidden" name="back_trips[{{$i}}][old_started]" value="{{$old_started}}">
            
            <div class="row">
                <div class="col-4">
                    <select name="back_trips[{{$i}}][st_hour]" class="form-control">
                ...
                <div class="col-4">
                    <select name="back_trips[{{$i}}][st_minute]" class="form-control">
            ...
            <div class="errors">
                @if ($st_time_error)
                    <span class="has-error">
                        {{session('time_arrive')?session('time_arrive'):"Thời gian xuất phát không hợp lệ"}}
                    </span>
                @endif
            </div>
    ...
    <div class="form-group row {{$et_time_error?'has-error':''}}" id="form-group-back_trips-{{$i}}-arrived">
        <label for="back_trips-{{$i}}-arrived_at" class="form-control-label col-sm-4 col-md-12 col-lg-4 col-xl-3" id="label-for-back_trips{{$i}}-started_at">Đến nơi sau...</label>
        <div class="input-datetime-wrapper col-sm-8 col-md-12 col-lg-8 col-xl-9">
            <input type="hidden" name="back_trips[{{$i}}][et_time]" value="{{$et_time}}">
            
            <div class="row">
                <div class="col-4">
                    <select name="back_trips[{{$i}}][et_day]" class="form-control">
                        @for ($j = 0; $j < 31; $j++)
                ...
                <div class="col-4">
                    <select name="back_trips[{{$i}}][et_hour]" class="form-control">
                        <option value="">Giờ</option>
                        @for ($j = 0; $j < 24; $j++)
                ...
                <div class="col-4">
                    <select name="back_trips[{{$i}}][et_minute]" class="form-control">
                        <option value="">Phút</option>
                        @for ($j = 0; $j < 60; $j+=5)
...
@else
    ...
    <div class="form-group row {{$ar_time_error?'has-error':''}}" id="form-group-back_trips-{{$i}}-started">
        <label for="back_trips-{{$i}}-arrived" class="form-control-label col-sm-4 col-md-12 col-lg-4 col-xl-3" id="label-for-back_trips{{$i}}-started">Vào bến</label>
        <div class="input-datetime-wrapper col-sm-8 col-md-12 col-lg-8 col-xl-9">
            <input type="hidden" name="back_trips[{{$i}}][old_arrived]" value="{{$old_arrived}}">
            
            <div class="row">
                <div class="col-4 auto">
                    <select name="back_trips[{{$i}}][ar_hour]" class="form-control">
                        <option value="">Giờ</option>
                        @for ($j = 0; $j < 24; $j++)
                ...
                <div class="col-4">
                    <select name="back_trips[{{$i}}][ar_minute]" class="form-control">
                        <option value="">Phút</option>
                        @for ($j = 0; $j < 60; $j+=5)
