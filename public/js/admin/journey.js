...
Cube.journey = {
    ...
    sortPlaces:function(places){
        Cube.ajax(this.urls.sort_places_url, "POST", { places:places }, function(rs) {
            if (rs.status) {
                //cl(rs.items);
            } else {
...
$(function() {
    if (typeof window.journeyInit == 'function') {
        window.journeyInit();
        window.journeyInit = null;
    }
    ...
    if($('#add-passing-place-form').length){
        $('#add-passing-place-form').submit(function(e){
            ...
            try{
                ...
                Cube.journey.addPassingPlace(url, data);
            }
            catch(error){
                cl(error.getMessage());
            }
    ...
    var updateOutput = function(e)
    {
        ...
        Cube.journey.sortPlaces(list.nestable('serialize'));
    };
