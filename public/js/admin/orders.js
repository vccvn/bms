...
Cube.orders = {
    ...
    listID: [],
    urls: {},
    status_list: { "600": "Đã hoàn thành", "300": "đang xử lý", "0": "Chờ xử lý", "-1": "Bị hũy" },
    status_color_types: {},
    templates: {},
    labels: {},
    ...
    changeStatus: function(id, status) {
        Cube.ajax(this.urls.change_status_url, "POST", { id: id, status: status }, function(rs) {
            if (rs.status) {
                var order = rs.order;
                if (order) {
                    var $btn = $('#order-item-' + order.id + ' .status-text');
                    ...
                    $('#order-item-' + order.id + ' .order-status-select .status-select a').removeClass('active');
                    $('#order-item-' + order.id + '-' + status).addClass('active');
                }
            } else {
                modal_alert("Đã có lỗi bất ngờ xảy ra. Vui lòng thử lại sau giây lát");
            }
        ...
        Cube.ajax(this.urls.view_url, 'GET', { id: id }, function(rs) {
            if (rs.status) {
                ...
                for (var label in this.labels) {
                    ...
                    else if(label == 'total_money'){
                        detail = (Cube.number.currency(parseInt(detail)))+' VN0';
                    }
